...
В третьем терминале:
```
while true; do PGPASSWORD='do3Eec7bahChah8uth' psql -h 192.168.0.178 -U postgres -t -c "SELECT inet_server_addr();" 2>/dev/null | head -1; sleep 1; done
```
В еще одном терминале подключиться к `haproxy-node1`, отключить службу `haproxy`:
```
sudo systemctl stop keepalived
```
VIP переехал на `haproxy-node2`, однако при отключении службы `haproxy` VIP не переезжает
